...
#from templates_translate import get_trans as get_templatestrans


try:
    from utils.wordlist import Wordlist,Word
except:
    from wordlist import Wordlist,Word  # 为了单独调试
...
class Trie:
    ...
    def __init__(self, path_or_trie=None):
        ...
        self.max_log_items = 0 # 最大词频
        """
        if isinstance(path_or_trie, str):  # 从文件中加载，文件的每一行是“词 词频
            with open(path_or_trie, encoding="utf8") as f:
                for l in f:
                    l = re.split(' +', l.strip())
    ...
    def __setitem__(self, item, count):
        ...
        if self.end in _:  # 调整词频
            ...
            rawcount =  _[self.end][1]
            _[self.end] = (item,rawcount+ count)
        ...
        log_count = log(count)
        if self.max_log_items<log_count:
            self.max_log_items = log_count
...
class Template:
    ...
    def get_trans(self):
        ...
        """
        return
        trans_res = get_templatestrans(self)
        self.__tran = trans_res
    ...
    def __str__(self, i = 0):
        ...
        for word in self.words:
            if word == None:
                aa.append(self.placeholder[i%5])
                i+=1
            else:
                aa.append(word)
    ...
    def __repr__(self):
        return u'< Template: %s >' % (self.__str__())
    ...
    def texts(self, sents, filtering=True):
        if filtering:
            for sent in sents:
                yield [
                    self.tokenize(s)
                    for s in re.split(u'[^\s\u4e00-\u9fa50-9a-zA-Z]+', sent) if s
                ]
        else:
            for sent in sents:
                yield [
...
class XTrie(Trie):
    ...
    def LoadTemplatesfromFile(self, file):
        """
        已文件格式如下的模板文件中加载模板
        ('formerly [X]', ('以前[X]', 5))
        """
        with open(file, "r", encoding="utf8" ) as Templates_file:
            for Template in Templates_file:
                _, num, word = parseTemplate(Template)
                self[word] = num
    ...
    def _search(
        ...
        if sent:
            ...
            if (not matching  # matching为空，即为起点
                    or matching[-1] != out_start + in_start-1  # 这意味着前面匹配了占位符
                ):
...
class SentTree:
    ...
    def __init__(self, parsing, words, start, end, tempdic):
        """SentTree有三个属性：
        template是当前主模版，
        content是当前主模版覆盖的字符串
        ...
        for i in range(len(marks) - 1):
            if marks[i] + 1 < marks[i + 1]:
                # 递归创建
                st = SentTree(parsing[1][k], words, marks[i] + 1, marks[i + 1],tempdic)
    ...
    def getModelnum(self):
        ...
        for md in self.modules:
            mdnum+=md.getModelnum
        return mdnum

    def templatelist(self,words,xtrie):
        """
        对于一个句子被解析为多个平行模板 进行解析：
        比如 A[X]  B[X]  到 （A[X]）（  B[X]）
        """
        length=len(words)
        start = 0
        tem = []
        ...
        while start != length:
            last_end = start
            rag = range(start+1, length+1)
            addtag = 0
            for end in rag:
                if str(Template(words[start:end])) in xtrie:
                    if addtag == 1:
                        tem.pop()
                    tem.append(Template(words[start:end]))
                    addtag = 1
                    last_end  = end
            if last_end == start: # 未找到
                if words[start] == None:
                    if len(tem)==0 or tem[-1] !=None:
                        tem.append(words[start])
                        start+=1
                else:
                    loop+=1
                    if loop >5:
    ...
    def get_templatestrans(self, templatedict):

        if str(self.template) in templatedict:
            self.template.trans_res = templatedict[str(self.template)][0]
        elif self.modules == []:
             pass #self.template.get_trans()
        else:
            trans = []
            tem = self.templatelist(self.template.words, templatedict)
            i=0
            if tem == "error":
                self.template.trans_res = "error"
                return
            for t in tem:
                if t == None:
                    trans.append(self.template.placeholder[i])
                    i+=1
                else:
                    subt = templatedict[str(t)]
                    len_Nonepos=len(t.Nonepos)
                    for ph in range(len_Nonepos):
                        old = self.template.placeholder[ph]
                        new = self.template.placeholder[ph+i]
                        subt = subt[0].replace(old,new)
                        trans.append(subt)
                    i+=len_Nonepos
            self.template.trans_res = " ".join(trans)
        for md in self.modules:
            md.get_templatestrans(templatedict)
        return

    def sent_tran(self, senttree=None,idx=0,tranlist=list()):
        uniqu_list = set()
        if senttree==None:
            senttree = self
        placeholderlist = ["X","Y","Z","M","N"]
        placeholder_num = len(senttree.template.Nonepos)
        ...
        if trans == "":
            ...
            trans = translateBybaidu(tranlist.SourceContext)
            return trans,0,-1
        subtrans = []
        _idx = 0
        dd = 0
        for md,mdidx in zip(senttree.modules,senttree.template.Nonepos):
            subtran,idx_delta,_ = self.sent_tran(md,idx=mdidx+idx+_idx-dd,tranlist=tranlist)
            dd+=1
            if _ == -1:
                return subtran,0,-1
            uniqu_list.update(_)
            _idx+=idx_delta
            subtrans.append(subtran)
        for i in range(placeholder_num):
            placeholder = "[{}]".format(placeholderlist[i])
            trans = trans.replace(placeholder, subtrans[i])
        return trans,senttree.template.length+_idx,uniqu_list



    def __len__(self):
         return len(self.template.words)
    ...
    def line(self):
        ...
        for w in self.template:
            if w == None:
                s += '( ' + self.modules[k].line() + ') '
                k += 1
            else:
                s += " "+w+" "
        return s

    def toWordlist(self,isRoot = True):
        """
        if self.__len__()==1 and isRoot == False:
            wrd = Word(self.content)
            wrd.trans_level = 1 # wrd.transContext = self.template.trans_res
            return wrd
        """
        _list = Wordlist([])
        sub_wordlist_num = 0
        tran_has_deliver = False
        if self.template.trans_res:
            transTag = 2
        for w in self.template:
            if w == None:
                wrdl = self.modules[sub_wordlist_num].toWordlist(False)
                wrdl.trans_level = 1
                _list.append(wrdl)
                sub_wordlist_num += 1
            else:
                wrd = Word(w)
                if not tran_has_deliver:
                    wrd.transContext = self.template.trans_res
                    tran_has_deliver = True
                _list.append(wrd)
        return _list
...
class Parser:
    ...
    def __init__(self, templates, tempdic, tokenize, null_score=0):
        self.templates = templates  # 模版trie树，需要XTrie类型
        self.tokenize = tokenize  # 分词函数
        if null_score==0:
            self.null_score = log(1/(2*templates.total_items))  # 不成模版的词的打分
        else:
            self.null_score = null_score
        self.tempdic = tempdic # dic 形式贮存的模板

    def max(self, x, num=1):  # 自定义最大值函数
        """
        返回  间隔 与 分数
        先比较概率，在按分隔符的位置
        """
        if x:
            return sorted(x, key=lambda s: (-round(s[1], 4), -len(s[0][0])) )[:num]
        else:
            return [(-1, 0)]

    def parse(self, sent, num=10,withscore =True):
        ws = self.tokenize(sent)
        matches = self.templates.search(ws)
        # print(ws)
        p = self._parse((0, len(ws)), matches, num)
        if withscore:
            return [ (SentTree(pitem[0], ws, 0, len(ws),self.tempdic),pitem[1]) for pitem in p]  # 返回（SentTree对象，分数）列表
        else:
            return [ SentTree(pitem[0], ws, 0, len(ws),self.tempdic) for pitem in p] # 返回SentTree对象列表

    def _parse(self, span, matches, num=1,level = 1):
        """解析思路：递归进行——如果一种层次结构是最优的，那么它每个语义块的
        层次结构都是最优的。利用这种特性，可以写成递层优化的算法。
        """
        tpls = []  # 候选层次结构集，每个元素代表一种层次结构
        # 格式为[((主模版间隔词idx集, [语义块1, 语义块2]), 得分)]
        # 语义块1、语义块2依然为
        # (语义块主模版间隔词idx集, [语义块的语义块1, 语义块的语义块2])
        # 的格式，即无限递归直到遍历完句子

        if span in matches: # 意味着当前语义块能析出模版
            moduleslist = [] # 不同模板有不同的语言块结构
            for t, f in matches[span].items(): # 遍历解析的每一个模板和频率
                # print(("(t, f):",(t, f)) )
                score = log(f) - log(self.templates.total_items)  # 主模版的对数频率
                # print(("score:",score))
                modules = []  # 初始化子语义块
                ...
                for i in range(len(marks) - 1):  # 枚举每个语义块
                    if marks[i] + 1 < marks[i + 1]:
                        # 如果不满足，则是连续的两个词，
                        # 则不是间隔，无法构成语义块。
                        # print((" (marks[i] + 1, marks[i + 1]):", (marks[i] + 1, marks[i + 1])))
                        _tpls_score_list = self._parse(
                            (marks[i] + 1, marks[i + 1]), matches,level=level+1)
                        _tpls = [tpl_score[0] for tpl_score in _tpls_score_list]
                        _score = [tpl_score[1] for tpl_score in _tpls_score_list]
                        modules.append(_tpls[0])
                        # print((" (_tpls, _score):", (_tpls[0], _score[0])))
                        score += _score[0]  # 主模版的对数频率+每个语义块的对数频率等于总对数频率
                # print(("modules:",modules))
                score = score*(1+(level-1)*0.1)
                tpls.append( (
                                (t, modules),
                                score
                            ) )
            return self.max(tpls, num)  # 返回前num个最优结构((主模版间隔词idx集, [语义块1, 语义块2]), 得分)
        else:  # 意味着没有模版能解析当前语义块，那么将它视为一系列孤立的词
            return [((tuple(range(span[0], span[1])), []),
                    self.null_score * (span[1] - span[0]))]
